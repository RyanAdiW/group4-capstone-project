...
func (ar *authRepo) LoginEmail(email, password string) (string, error) {
	result, err := ar.db.Query("select id, name, email, birth_date, phone_number, photo, gender, address FROM users WHERE email=? AND password=?", email, password)
	if err != nil {
		return "", err
	}
	...
	errScan := result.Scan(&user.Id, &user.Name, &user.Email, &user.Birth_date, &user.Phone_number, &user.Photo, &user.Gender, &user.Address)
	if errScan != nil {
		return "", errScan
	}
	token, err := _middlewares.CreateToken(user.Id, user.Email)
	if err != nil {
		return "", err
	}
	return token, nil
